...
ecoreg1 <- terra::vect(ecoreg)
ecoreg1<- terra::rasterize(ecoreg1,clim[[1]],field='ECO_ID') #in order to crop
...
l=foreach(spi = sptod ,
          ...
          .errorhandling = 'pass') %dopar% {
            ...
            projectRangeBag=function(env1,rbm){
              ...
              newEnv=raster::values(env1) #in case of abortion
            ...
            if (nrow(occ)<= 1000) {
              ...
              if (sum(studyarea$SHAPE_AREA.1)<= 500) {
                ...
                clim = raster::stack(mixedsort(bioVars))
                clim <- subset(clim,c(1,5:14,16:19))
                climname <- names(clim)
                study <- raster::crop(ecoreg1,studyarea,mask=TRUE,snap='out') #crop to the studyarea
                study <- raster::raster(study)
                speciesEcoID = studyarea$ECO_ID.1
                reclMatrix = matrix (data = c(speciesEcoID,rep(1,length.out=length(speciesEcoID))),
                                     ncol = 2,byrow = F )
                study = raster::reclassify (study, rcl=reclMatrix,othersNA=TRUE)
                clim <- raster::crop(clim,study,snap="out",mask=T) #crop to the extent of study area
                clim <- clim * study  #only keep climatic data in selected study area
                names(clim) <- climname
                #plot(clim)
                #plot(study)
                ...
                occClim = raster::extract(clim,occ[,c(1,2)])
                ...
                #model fit====  
                rbOcc <- rangeBag(x=occClimCalibrate[,c(1:15)])
                ...
                study <- terra::rast(study)
                bkgClim = terra::as.data.frame (study,cells=T,na.rm=T)
                ...
                coo <- terra::xyFromCell(study,idsBkg)
                ...
                predsBkg = raster::extract(outputProb,climBkg[,c(2,3)])
                ...
                rm(list = c('clim','occ','occ_sf',"studyarea" ,"study",
