...
spPRan = list.files(pattern = '.stats.rds$',
                   path = 'D:/quercus/Partage/Pengcheng/R/SDM/potential_geographical_range/1',
                   recursive = T)

#set up working directory 
setwd("D:/quercus/Partage/Pengcheng/R/SDM/potential_geographical_range/1")
sta_ls = lapply(spPRan, readRDS)
sta_ls <- bind_rows(sta_ls)
# get species name that auc is below 0.8 and need to redo the rb model(for the sake of projection)
spPRan <- sta_ls[sta_ls$AUC <= 0.8,"spName"]
...
#id number of species with potential geographical range among species 
#with geographical range
sptodo <- spOcc[(spOcc %in% spPRan)]
...
l=foreach(spi = sptod,
          ...
          .errorhandling = 'pass') %dopar% {
            ...
            if (nrow(occ)> 1000) { #need to change the value for auc improvement
              ...
              if (sum(studyarea$SHAPE_AREA.1)> 500) { #need to change the value for auc improvement
                ...
                #output 
                outputSpDir = paste0('D:/quercus/Partage/Pengcheng/R/SDM/potential_geographical_range/1/',spName)
